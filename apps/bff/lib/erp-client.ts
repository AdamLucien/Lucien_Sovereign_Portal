...
const LUCIEN_TIER_FIELD = process.env.LUCIEN_TIER_FIELD?.trim();

export const isErpConfigured = () => {
  return Boolean(ERP_BASE_URL && ERP_API_KEY && ERP_API_SECRET);
};

export type DataMode = 'erp' | 'mock';

export const getDataMode = (): DataMode => (isErpConfigured() ? 'erp' : 'mock');
...
const resourcePath = (doctype: string) => {
...
export const realErpClient: ERPClient = {
  async fetchProjects() {
  ...
  async fetchProjectById(projectId: string) {
    try {
      const payload = await erpJson(`${resourcePath('Project')}/${encodeURIComponent(projectId)}`);
...
export const mockErpClient: ERPClient = {
  async fetchProjects() {
    return mockProjects;
  },
  async fetchProjectById(projectId: string) {
  ...
  async updateClientRequestStatus(requestId: string, status: string) {
    const request = mockClientRequests.find((entry) => entry.name === requestId);
    if (request) {
  ...
  async updateDirectiveAck(directiveId: string, payload: { ack_by: string; ack_at: string }) {
    const directive = mockDirectives.find((entry) => entry.name === directiveId);
    if (directive) {
      directive.ack_by = payload.ack_by;
      directive.ack_at = payload.ack_at;
    }
  },
  async fetchLatestInvoice(projectId: string) {
    const invoices = mockInvoices.filter((invoice) => invoice.project === projectId);
    if (!invoices.length) return null;

    return invoices.reduce((latest, current) => {
      const latestDate = toTimestamp(latest.posting_date) || toTimestamp(latest.due_date) || 0;
      const currentDate = toTimestamp(current.posting_date) || toTimestamp(current.due_date) || 0;
      return currentDate >= latestDate ? current : latest;
    }, invoices[0]);
  },
  async fetchInvoicesByProject(projectId: string) {
    return mockInvoices
      .filter((invoice) => invoice.project === projectId)
      .sort((a, b) => {
        const aDate = toTimestamp(a.posting_date) || toTimestamp(a.due_date) || 0;
        const bDate = toTimestamp(b.posting_date) || toTimestamp(b.due_date) || 0;
        return bDate - aDate;
      });
  },
  async fetchContractsByProject(projectId: string) {
...
export const erpClient: ERPClient = isErpConfigured() ? realErpClient : mockErpClient;
